...
const NoticeList = ({match, history}) => {
    ...
    const handleSelectChange = value => {
        history.push(`/student/notice/list/1`)
    ...
    const onClickNext = () => {
        history.push(`/student/notice/list/${pager.nextBlock + 1}`)
        transPage(selectedCate, pager.nextBlock)
    }
    const onClickPrev = () => {
        history.push(`/student/notice/list/${pager.preBlock+1}`)
        ...
        <CardBody>
                ...
                {list.map((i, index) => (
                    <tr key={index}>
                        ...
                        <td className={"t-notice-textAlign-left"}><NavLink
                            to={`/student/notice/detail/${i.id}`}>{i.title}</NavLink>
                            <span className="m-2 s-notice-comment">[{i.commentCount}]</span>
                        </td>
            ...
            <div className="btn-cover text-center">
                {isVisible ? <>
                        ...
                        {pages.map((i, index) =>
                            <NavLink to={`/student/notice/list/${i}`} key={index}>
                                <button onClick={() => {
